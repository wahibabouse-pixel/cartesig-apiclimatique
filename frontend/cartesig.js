...
const I18N = {
  ar: {
    ...
    fs_title:         '🌾 المستثمرات',
    fs_search_ph:     'البحث في المستثمرات...',
    fs_empty:         'لا توجد مستثمرات',
    fs_no_results:    'لا توجد نتائج',
    fs_loading:       'جارٍ التحميل...',
    fs_item_meta:     (id, surf) => `#${id}${surf ? ' · ' + surf + ' هـ' : ''}`,
    // Drawer sections
  ...
  fr: {
    ...
    fs_title:         '🌾 Exploitations',
    fs_search_ph:     'Rechercher une exploitation...',
    fs_empty:         'Aucune exploitation',
    fs_no_results:    'Aucun résultat',
    fs_loading:       'Chargement...',
    fs_item_meta:     (id, surf) => `#${id}${surf ? ' · ' + surf + ' ha' : ''}`,
    // Drawer sections
...
function applyLang() {
  const lng = I18N[LANG];
  document.documentElement.lang = lng.lang_code;
  document.documentElement.dir  = lng.lang_dir;
  ...
  document.title = LANG === 'ar' ? 'خريطة SIG الفلاحية – الجزائر' : 'Carte SIG Agricole – Algérie';
  // update fs search placeholder
  const fsSearch = document.getElementById('fs-search');
  if (fsSearch) fsSearch.placeholder = lng.fs_search_ph;
}
...
// ─── State ───
let allFarms = [];        // كل مستثمرات البلدية المحددة
let farmMarkers = {};     // { farmId: { marker, farm, lat, lng } }
let activeFarmId = null;  // المستثمرة المحددة حالياً في القائمة
...
let selWLayer = null, selCLayer = null;
let selW = null, selC = null;
let WLIST = [], cFull = [];

const STYLE_DEFAULT  = { color:'#9a975b', weight:1.8, opacity:.9, fillOpacity:.08, fillColor:'#1c5433' };
const STYLE_SELECTED = { color:'#d4af37', weight:3,   opacity:1,  fillOpacity:.18, fillColor:'#3da860' };
    ...
    .then(data => {
      WILAYAS_LAYER = L.geoJSON(data, {
        ...
        onEachFeature: (feature, layer) => {
          ...
          layer.on('click', () => map.flyToBounds(layer.getBounds(), { padding:[20,20], duration:.8 }));
        }
      }).addTo(map);
    })
    .catch(() => showToast(t('err_geojson')));
}
...
function filterW(q) {
  ...
  return WLIST.filter(w => w.nom_fr.toLowerCase().includes(lq)||(w.nom_ar||'').includes(q)||w.code===q).map(toWItem);
}
...
async function selectW(wid, nfr, nar) {
  ...
  if (geo) {
    map.flyToBounds(geo.bounds, { padding:[40,40], duration:.85 });
    centerLat=geo.center.lat; centerLng=geo.center.lng;
  } else {
    ...
    if (geoNom) {
      ...
      map.flyToBounds(selWLayer.getBounds(), { padding:[40,40], duration:.85 });
      const c = selWLayer.getBounds().getCenter();
      centerLat=c.lat; centerLng=c.lng;
    } else {
      map.flyTo([28,2.5], 6, { duration:.85 });
...
async function selectC(comm) {
  ...
  if (geoNom) {
    ...
    map.flyToBounds(selCLayer.getBounds(), { padding:[50,50], duration:.75 });
    const c = selCLayer.getBounds().getCenter();
    clat=c.lat; clng=c.lng;
  } else {
    clat=28; clng=2.5;
    map.flyTo([clat, clng], 11, { duration:.75 });
  }
  ...
  try {
    ...
    farms.forEach((f, i) => {
      const fLat = f.latitude  ? parseFloat(f.latitude)  : clat + (Math.random()-.5)*.08;
      const fLng = f.longitude ? parseFloat(f.longitude) : clng + (Math.random()-.5)*.08;
      ...
      markerEl.className = 'fm';
      markerEl.style.cssText = `width:34px;height:34px;animation-delay:${i*55}ms`;
      ...
      mk.on('click', () => {
        setActiveFarm(f.id);
        openFarmPopup(mk, f, fLat, fLng);
      });

      farmMarkers[f.id] = { marker:mk, farm:f, lat:fLat, lng:fLng, markerNode:null };
    });
    ...
    // ── عرض قائمة المستثمرات ──
    showFarmsSection(farms);
...
//  FARMS LIST SECTION
// ═══════════════════════════════════════════════
function showFarmsSection(farms) {
  farmsSection.classList.add('visible');
  fsCount.textContent = farms.length;
  fsSearch.value = '';
  renderFarmList(farms);
}

function hideFarmsSection() {
  farmsSection.classList.remove('visible');
  fsList.innerHTML = `<div class="fs-empty" data-i18n="fs_empty">${t('fs_empty')}</div>`;
...
function renderFarmList(farms) {
  if (!farms.length) {
    fsList.innerHTML = `<div class="fs-empty">${t('fs_no_results')}</div>`;
    return;
  }
  fsList.innerHTML = '';
  farms.forEach(f => {
    const name = LANG==='ar'
      ? (f.nom_exploitation_ar||f.nom_exploitation_fr||I18N[LANG].farm_label(f.id))
      : (f.nom_exploitation_fr||f.nom_exploitation_ar||I18N[LANG].farm_label(f.id));
    const surf = f.superficie_batie
      ? parseFloat(f.superficie_batie).toFixed(1)
      : null;
    const meta = I18N[LANG].fs_item_meta(f.id, surf);

    const item = document.createElement('div');
    item.className = 'farm-item' + (activeFarmId===f.id ? ' active-farm' : '');
    item.dataset.fid = f.id;
    item.innerHTML = `
      <div class="farm-ic">🌾</div>
      <div class="farm-info">
        <div class="farm-name" title="${name}">${name}</div>
        <div class="farm-meta">${meta}</div>
      </div>
      <span class="farm-nav">◀</span>`;
    item.addEventListener('click', () => flyToFarm(f.id));
    fsList.appendChild(item);

    // حفظ مرجع العنصر في farmMarkers
    if (farmMarkers[f.id]) farmMarkers[f.id].listItem = item;
  });
}

function flyToFarm(farmId) {
  const entry = farmMarkers[farmId];
  if (!entry) return;
  setActiveFarm(farmId);
  map.flyTo([entry.lat, entry.lng], 15, { duration:.7 });
  setTimeout(() => {
    openFarmPopup(entry.marker, entry.farm, entry.lat, entry.lng);
  }, 750);
}

function setActiveFarm(farmId) {
  // إزالة التمييز السابق
  if (activeFarmId && farmMarkers[activeFarmId]) {
    const prev = farmMarkers[activeFarmId];
    if (prev.listItem) prev.listItem.classList.remove('active-farm');
  }
  activeFarmId = farmId;
  const curr = farmMarkers[farmId];
  if (curr && curr.listItem) {
    curr.listItem.classList.add('active-farm');
    curr.listItem.scrollIntoView({ behavior:'smooth', block:'nearest' });
  }
}

// بحث في قائمة المستثمرات
fsSearch.addEventListener('input', () => {
  const q = fsSearch.value.trim().toLowerCase();
  if (!q) { renderFarmList(allFarms); return; }
  const filtered = allFarms.filter(f => {
    const nameAr = (f.nom_exploitation_ar||'').toLowerCase();
    const nameFr = (f.nom_exploitation_fr||'').toLowerCase();
    return nameAr.includes(q) || nameFr.includes(q) || String(f.id).includes(q);
  });
...
function renderDrawer(d) {
  ...
  let html  = '';
  html += section(lng.dr_info,`<div class="dr-grid">${card(lng.dr_campagne,r.campagne)}${card(lng.dr_date,r.date_recensement)}${card(lng.dr_status,r.status)}${card(lng.dr_year,r.annee)}</div>`);
  if (d.superficies?.length) html += section(lng.dr_surf,`<div class="dr-grid">${d.superficies.map(s=>card(s.type,`${lng.dr_dry}: ${s.surface_sec} | ${lng.dr_irrigated}: ${s.surface_irrigue}`)).join('')}</div>`);
  if (d.cultures?.length)    html += section(lng.dr_cult,`<div class="dr-grid">${d.cultures.map(c=>card(c.type,`${lng.dr_dry}: ${c.surface_sec} | ${lng.dr_irrigated}: ${c.surface_irrigue}`)).join('')}</div>`);
  if (d.arbres?.length)      html += section(lng.dr_trees,`<div class="dr-grid">${d.arbres.map(a=>card(a.type,`${a.nombre} ${lng.dr_tree_unit}`)).join('')}</div>`);
  if (d.cheptel?.length)     html += section(lng.dr_cheptel,`<div class="dr-grid">${d.cheptel.map(c=>card(c.animal,`${c.nombre_total} (${lng.dr_female}: ${c.nb_femelle})`)).join('')}</div>`);
  if (d.bovins?.length)      html += section(lng.dr_bovins,`<div class="dr-grid">${d.bovins.map(b=>card(b.categorie,b.nombre)).join('')}</div>`);
  if (d.aviculture?.length)  html += section(lng.dr_poultry,`<div class="dr-grid">${d.aviculture.map(a=>card(a.type,`${LANG==='ar'?'بياض':'Ponte'}: ${a.nb_ponte} | ${LANG==='ar'?'لحم':'Chair'}: ${a.nb_chaire}`)).join('')}</div>`);
  if (d.apiculture?.length)  html += section(lng.dr_bees,`<div class="dr-grid">${d.apiculture.map(a=>card(a.ruche,`${a.nb_ruches_total} (${LANG==='ar'?'ممتلئة':'Pleines'}: ${a.nb_ruches_pleine})`)).join('')}</div>`);
  if (d.irrigation?.length)  html += section(lng.dr_irrigation,`<div class="dr-grid">${d.irrigation.map(i=>card(i.mode,`${i.surface} ${lng.dr_ha}`)).join('')}</div>`);
  if (d.batiments?.length)   html += section(lng.dr_buildings,`<div class="dr-grid">${d.batiments.map(b=>card(b.type,`${b.nombre} × ${b.surface} ${lng.dr_m2}`)).join('')}</div>`);
  if (d.materiels?.length)   html += section(lng.dr_equipment,`<div class="dr-grid">${d.materiels.map(m=>card(m.type,m.nombre)).join('')}</div>`);
  if (d.menage_agricole) {
    const mg=d.menage_agricole;
    html += section(lng.dr_household,`<div class="dr-grid">${card(lng.dr_persons,mg.nb_personnes)}${card(lng.dr_adults,mg.nb_adultes)}${card(lng.dr_children,mg.nb_enfants)}</div>`);
  }
  if (d.intrants) {
    ...
    if (tags.length) html += section(lng.dr_inputs, tags.map(tg=>`<span class="tag">${tg}</span>`).join(''));
  }
  return html;
}
...
langBtn.addEventListener('click', () => {
  LANG = LANG==='ar' ? 'fr' : 'ar';
  applyLang();
  if (!ci.disabled && cFull.length) ci.placeholder = I18N[LANG].ph_commune_ready(cFull.length);
  // إعادة رسم قائمة المستثمرات إذا كانت ظاهرة
  if (allFarms.length) {
    renderFarmList(allFarms);
    document.querySelector('.fs-title') && (document.querySelector('.fs-title').textContent = I18N[LANG].fs_title);
  }
...
function resetMap() {
  ...
  if (selCLayer) { map.removeLayer(selCLayer); selCLayer=null; }
  farmLG.clearLayers();
  farmMarkers={}; allFarms=[]; activeFarmId=null;
  selW=null; selC=null;
  ...
  hideFarmsSection();
  map.flyTo([28,2.5], 5, { duration:.85 });
